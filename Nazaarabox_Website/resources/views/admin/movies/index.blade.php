...
@section('title', 'Movies Management - Admin Panel')
        ...
        <div style="max-width: 1280px; margin: 0 auto; padding: 0 16px;">
            <div style="display: flex; justify-content: space-between; align-items: center; height: 64px;">
                <div style="display: flex; align-items: center; gap: 24px;">
                    ...
                    <h1 style="font-size: 20px; font-weight: bold; color: white; margin: 0;">Movies Management</h1>
                </div>
                <div style="display: flex; align-items: center; gap: 16px;">
                    <button onclick="window.location.href='/admin/movies/create'" class="flex items-center gap-2 bg-primary-600 hover:bg-red-700 text-white px-4 py-2 rounded transition-colors text-sm">
    ...
    <main style="max-width: 1280px; margin: 0 auto; padding: 32px 16px;">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="stat-card p-6">
                <p style="font-size: 14px; color: #9ca3af; margin-bottom: 8px;">Total Movies</p>
        ...
        <!-- Movies Grid -->
        <div id="movies-container" style="display: none;">
        ...
        <div id="empty-state" style="display: none; text-align: center; padding: 64px 16px;">
            <p style="color: #9ca3af; font-size: 18px; margin-bottom: 16px;">No movies found</p>
            <button onclick="window.location.href='/admin/movies/create'" class="bg-primary-600 hover:bg-red-700 text-white px-6 py-2 rounded transition-colors">
...
<script>
    ...
    async function fetchMovies(page = 1) {
        ...
        let url = `${API_BASE_URL}/admin/movies?page=${page}`;
    ...
    function renderMovies(movies) {
        ...
        movies.forEach(movie => {
            ...
            card.onclick = () => window.location.href = `/admin/movies/${movie.id}`;
            ...
            card.innerHTML = `
                <div style="position: relative; margin-bottom: 8px;">
                    ...
                    <div style="position: absolute; bottom: 4px; left: 4px; background-color: rgba(0,0,0,0.8); color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px; font-weight: 500; line-height: 1.2;">
                        ${(movie.status || 'N/A').substring(0, 3).toUpperCase()}
                    </div>
                </div>
                <h3 style="color: white; font-weight: 600; margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-size: 13px; line-height: 1.3;">
                    ${movie.title || 'Untitled'}
                </h3>
                <p style="color: #9ca3af; font-size: 11px; margin: 0 0 4px 0;">
                    ${movie.release_date ? new Date(movie.release_date).getFullYear() : 'N/A'}
                </p>
                <div style="display: flex; gap: 6px; font-size: 10px; color: #9ca3af; line-height: 1.2;">
    ...
    function renderPagination(pagination) {
        ...
        if (startPage > 1) {
            html += `<button onclick="fetchMovies(1)" class="bg-dark-700 hover:bg-dark-600 text-white px-4 py-2 rounded transition-colors" style="background-color: #2a2a2a; padding: 8px 16px; border-radius: 6px; border: 1px solid #3a3a3a;">1</button>`;
        ...
        if (endPage < totalPages) {
            ...
            html += `<button onclick="fetchMovies(${totalPages})" class="bg-dark-700 hover:bg-dark-600 text-white px-4 py-2 rounded transition-colors" style="background-color: #2a2a2a; padding: 8px 16px; border-radius: 6px; border: 1px solid #3a3a3a;">${totalPages}</button>`;
        }
